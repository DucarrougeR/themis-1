...
class Content:
    ...
    def validate_content(self):
        ...
        for i in range(len(error)):
            ...
            if error[i].look:
                looks_error.append("Look {} titled \"{}\" has this error: {}".format(error[i].look.id,
                                                                                     error[i].look.title,
                                                                                     error_msg))
            elif error[i].dashboard:
                dash_errors.append("Dashboard {} titled \"{}\" has this error: {}".format(error[i].dashboard.id,
                                                                                          error[i].dashboard.title,
                                                                                          error_msg))
            else:
