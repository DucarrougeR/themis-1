...
class Connectivity:
    ...
    def test_db_connections(self):
        ...
        all_connections = self.looker_client.all_connections(fields='name, port, host, dialect(connection_tests, label)')
        db_errors = []
            ...
            if elem.enabled:
                try:
                    ...
                    if not integration_test.success:
                        integration_errors.append("FAILED - Integration {} connectivity test".format(elem.label))
                    else:
